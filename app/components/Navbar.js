    ...
    <nav className={scrolling ? 'navbar-scroll max-container padding-container flex justify-between py-8 fixed z-50' : 'max-container padding-container flex justify-between py-8'}>
        <div className='left'>
            <Link href='/' className='flexCenter gap-2'>
                ...
                <h2 className='font-bold text-lg'>HopAround</h2>
            </Link>            
        </div>

        <div className='middle'>
            <ul className="hidden h-full gap-12 lg:flex">
                {NAV_LINKS.filter(link => link.key !== 'signin' && link.key !== 'signup').map((link) => (
        ...
        <div className='right lg:flexCenter hidden gap-4'>
            <Button type='button' title='Login' href = './auth/sign-in' variant='btn_white' />
            <Button type='button' title='Sign Up' href = './auth/sign-up' variant='btn_purple' />
        </div>         
